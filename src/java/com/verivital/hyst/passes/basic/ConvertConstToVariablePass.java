...
public class ConvertConstToVariablePass extends TransformationPass
{
	...
	protected void runPass(String params)
	{
		...
		if (value != null)
		{
			Operation le = new Operation(Operator.LESSEQUAL, new Variable(varName), new Constant(value.max));
			Operation ge = new Operation(Operator.GREATEREQUAL, new Variable(varName), new Constant(value.min));
		...
		if (index == -1)
		{
			if (nc.constants.containsKey(name))
				throw new AutomatonExportException("Constant not found, but conversion desired: " + name 
						+ " in component " + nc.instanceName);
			
			rv = nc.constants.get(name);
			
			changeMapping(nc, name);
			setVariableDynamics(nc, name, new ExpressionInterval(new Constant(0))); // add it as a variable....
		}
		else
		{
	 ...
	 * @param c the component where to set the variable
	...
	private void changeMapping(NetworkComponent nc, String name)
	{
		...
		for (ComponentInstance ci : nc.children.values())
		{
			...
			while (mapIt.hasNext())
			{
				...
				{
					ci.varMapping.add(map); // add this const mapping to the var mapping
