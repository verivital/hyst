...
/**
 * This transformation pass performs automaton flattening. Usually you want to call flattenAndOptimize, which
 * will flatten, remove discretely-unreachable modes, and convert havoc dynamics. If you instead call the pass 
 * directly you'll only get the flattening (modes are a cross product of the subcomponents), and havoc dynamics
 * may not be resolved.
 * 
 * Modes get renamed based on the modes in each base component, separated by SEPARATOR('_'). This can lead to 
 * ambiguous names if modes already have underscores in them; this isn't handled currently.
 */
public class FlattenAutomatonPass extends TransformationPass
{
	...
	/**
	 * Perform automaton flattening, as well as removing (discrete) unreachable states. This guarantees that:
	 * 
	 * - the root component of the configuration is a BaseComponent
	 * - every variable must have a defined flowDyanmics in every nonurgent mode (havoc dynamics are converted)
	 * 
	 * @param c the configuration to perform the passes on
	 */
	...
	private BaseComponent flatten(Component c)
	{
		...
		else 
		{
			...
			if (nc.children.size() == 0)
				throw new AutomatonExportException("Network component with zero children are not allowed.");
	 ...
	 * @param rv the place to store the params
	 * @param from the place to take them from
	 */ 
	...
	private static void mergeParamMap(LinkedHashMap<String, Interval> rv, LinkedHashMap<String, Interval> from)
	{
		for (Entry<String, Interval> e : from.entrySet())
		{
			...
			if (rvVal == null)
				...
				throw new AutomatonExportException("identical constants but different mapped values (diff value)");
		}
	}
	...
	private static void mergeLocations(BaseComponent left, BaseComponent right,	BaseComponent rv)
	{
		for (AutomatonMode locI : left.modes.values())
		{
			...
			{
				String combinedName = locI.name + SEPARATOR + locJ.name; // dots will be eliminated later
				...
				else
				{
					try
					{
						merged.flowDynamics = mergeExpressionMap(locI.flowDynamics, locJ.flowDynamics);
					}
					catch (AutomatonExportException e)
					{
						throw new AutomatonExportException("Conflicting dynamics (multiple drivers) detected while " +
								"flattening automaton", e);
					}
				}
			}
		}
	 ...
	 * @param rv where the add transitions to
	 * @param comp which component to add from
	 * @param isLeft is this component left-composed in the location names?
	 * @param sharedLabels the list of shared labels (transitions with these labels are skipped)
	 */
	private static void addNonSharedTransitions(BaseComponent rv, BaseComponent comp, boolean isLeft, 
			Collection <String> sharedLabels)
	{
			...
			for (AutomatonMode locRv : rv.modes.values())
			{
				...
				if (matchedSource)
				{
					...
					for (AutomatonMode locRv2 : rv.modes.values())
					{
						...
						{
							Hyst.logDebug("Adding transition '" + locName + "' -> '" + locName2 + "'");
	...
	private static LinkedHashMap<String, ExpressionInterval> copyMap(LinkedHashMap<String, ExpressionInterval> from)
	{
		LinkedHashMap<String, ExpressionInterval> rv = new LinkedHashMap<String, ExpressionInterval>();
	 ...
	 * @param rv where the add the transitions to
	 * @param left the left-composed component
	 * @param right the right-composed component
	 * @param sharedLabels the labels shared between components
	 */
	private static void addSharedTransitions(BaseComponent rv, BaseComponent left, BaseComponent right, 
			Collection<String> sharedLabels)
	{
		for (AutomatonTransition leftT : left.transitions)
		{
			...
			for (AutomatonTransition rightT : right.transitions)
			{
				...
				// at this point labels match and we can determine the full source and target names
				...
				if (sourceMode == null)
					throw new AutomatonExportException("source name not found in modes: " + sourceName);
				
				AutomatonMode targetMode = rv.modes.get(targetName);
				
				if (targetMode == null)
					throw new AutomatonExportException("target name not found in modes: " + targetName);
				...
				{
					throw new AutomatonExportException("Conflicting reset (multiple drivers) detected while " +
							"flattening automaton", e);
				}
			}
		}
	...
	private static BaseComponent mergeComponents(BaseComponent left, BaseComponent right)
	{
		...
		// merge the automata by first constructing the cross product of the mode names
