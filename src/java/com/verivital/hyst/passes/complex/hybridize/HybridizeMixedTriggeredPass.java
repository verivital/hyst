...
{
	@Option(name="-T", required=true, aliases = {"-maxtime"}, usage="The simulation time", metaVar="VAL")
	double timeMax;
	
	// The simulation strategy, S, one of {center|star|corners|starcorners|rand10|rand#}
	@Option(name="-S", aliases = {"-simstrat"},
			usage="The simulation strategy, S, one of {center|star|corners|starcorners|rand10|rand#}", 
				metaVar="SIMTYPE")
	...
	// The maximum simulation time before giving up on a space-triggered step, delta_pi
	@Option(name="-delta_pi", usage="The maximum simulation time before giving up on a space-triggered step", 
			metaVar="TIME")
	double piMaxTime = 0;
    ...
    private void makeParams()
    {
		...
		else if (simTypeString.startsWith("rand") && simTypeString.length() > 4)
		{
			...
			throw new AutomatonExportException("Unknown simulation type parameter: " + simTypeString);
    ...
    private ArrayList <SymbolicStatePoint> getSimulationStart()
    {
		...
		rv.add(new SymbolicStatePoint(initialMode, center)); // all sim types include center
	...
	private void runSimulation(ArrayList <SymbolicStatePoint> simPoints)
	{
		...
		while (elapsed + TOL < timeMax)
		{
			...
			Hyst.logDebug("simulation bounding box at step " + step + " was " + simBox + "; points were: " + simPoints);
	 ...
	 * the decided-upon hyperplane (within time piMaxTime).
	 *  
	...
	{
		HyperRectangle startBox = HyperRectangle.bloatAdditive(boundingBox(points(simPoints)), epsilon);
