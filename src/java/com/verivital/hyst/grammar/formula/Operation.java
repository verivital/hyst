...
public class Operation extends Expression 
{
	...
	public Operation(Operator operator, Expression... exps) 
	{
        ...
        public Operation(Operation parent,
			Expression left, Operator operator, Expression right) {
		super(parent);
		children.add(left);
	 ...
	 * @param left the right-hand-side expression
	...
	public String toStringInline()
	{
		...
		{
			throw new AutomatonExportException("No way defined to print expression with " + children.size() + " children.");
		}
	...
	{
		if (children.size() != 2)
			throw new AutomatonExportException("getLeft() is only valid when there are two children: " + this);
		if (children.get(0) != null) children.get(0).setParent(this);
	...
	{
		if (children.size() != 2)
			throw new AutomatonExportException("getRight() is only valid when there are two children: " + this);
		if (children.get(1) != null) children.get(1).setParent(this);
