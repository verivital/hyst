...
/**
 * Creates a chain of integrators. Based on "Chains of Integrators as a Benchmark 
 * for Scalability of Hybrid Control Synthesis", Livingston et. al, ARCH 2016 
 * 
 * Paramters are # of parallel chains (n)
 * Length of chain m
 * Input for the last dimension in each chain (u0 ... um)
 * 
 * timeVar, optional, (should an explicit 'time dimension be added? parameter is the name)
 * xi, input noise list, optional, the additive noise for each input
 * 
...
public class IntegralChainGenerator extends ModelGenerator 
{
	...
	@Option(name="-U",required=true,usage="input expression for each chain (list of length N), "
			+ "use parenthesis if there are parsing errors", 
			handler=ExpressionValueArrayOptionHandler.class, metaVar="U_0 U_1 ... U_N")
	private List<Expression> uList;
	
	@Option(name="-xi",usage="noise magnitude for each input expression (list of length N)", 
			handler=DoubleArrayOptionHandler.class, metaVar="xi_0 xi_1 ... xi_N")
	...
	protected Configuration generateModel() 
	{
		...
		// first variable in each chain is x_0, x_1, x_2, ... first der is x_0_der1, ect.
