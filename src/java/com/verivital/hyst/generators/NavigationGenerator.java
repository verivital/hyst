...
public class NavigationGenerator extends ModelGenerator 
{
	...
	@Option(name="-i_list",required=true,
			usage="space-separated list of target velocity i value for each mode, each i is 0-8 or 'A' or 'B'", 
	...
	private void checkParams()
	{
		...
		try
		{
			for (String s : iList)
			{
				...
				else
				{
					...
					if (i < 0 || i > 8)
						throw new AutomatonExportException("i_list argument was invalid: '" + i + "'");
		...
		{
			throw new AutomatonExportException("Error parsing i_list argument as integer " + e.toString(), e);
		}
		...
		if (iListProcessed.size() % width != 0)
			throw new AutomatonExportException("Width(" + width 
					+") should evenly divide number of elements in i_list (" + iListProcessed.size() + ").");
	...
	private void assignDerivative(AutomatonMode am, int i)
	{
		...
		else
		{
			...
			// v' = A(v-vd)
			Expression xvelDer = FormulaParser.parseValue(
					matrixA[0][0] + " * (xvel - " + desiredXvel + ") + " + 
					matrixA[0][1] + " * (yvel - " + desiredYvel + ")"); 
			Expression yvelDer = FormulaParser.parseValue(
					matrixA[1][0] + " * (xvel - " + desiredXvel + ") + " + 
					matrixA[1][1] + " * (yvel - " + desiredYvel + ")");
