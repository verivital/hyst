...
/**
 * Kodaik is a NASA tool for finding enclosures of solutions for bounded nonlinear equations.
 * It is run as a separate executable, expected to be on the PATH or KODIAK_PATH
 * 
 */
...
{	
	...
	/**
	 * Optimize a list of functions in a hyper-rectangle using kodiak
	 * This requires the Kodiac executable is on PATH or KODIAK_PATH, or null is returned
	 *
			...
			List<HashMap<String, Interval>> boundsList)
	{
		...
		{
			throw new AutomatonExportException("Error making Kodiak input file: " + e.toString(), e);
		}
	...
	private static Process openProcess(String inputFilename)
	{
		...
		if (loc != null)
		{
	        ...
	        {
	        	throw new AutomatonExportException("Exception while starting kodiak process: " 
	        			+ e.toString(), e);
	        }
		}
