	...
	private void accumulateExpressions(Expression original, Expression e, List <String> variables)
	{
		if (e instanceof Operation)
		{
			...
			else 
			{
				...
				{
					String text = aee.getMessage() + "; original whole expression: " + original.toDefaultString();
					throw new AutomatonExportException(text);
				}
			}
		}
		else
			throw new AutomatonExportException("Could not parse linear expression set from: " + original.toDefaultString() 
