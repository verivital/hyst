...
/**
 * Class for parsing a linear expression as a condition
 * Top level is expected to be an '=='
 * @author Stanley Bak
 *
 */
public class LinearExpression
{
	...
	public LinearExpression(Operation o, List <String> variables)
	{
		...
		if (o.op == Operator.EQUAL)
			...
			throw new AutomatonExportException("Could not extract linear condition from expression (bad operator): " + o);
		
		// ok, now we should only have pluses and minuses on each of the terms
		// let's keep all the terms on the left, and all the constants on the right
	...
	private void populateCoefficients(Expression e, boolean isLeftHandSide, List <String> vars)
	{
		...
		else if (e instanceof Operation)
		{
			...
			else if (o.op == Operator.MULTIPLY)
			{
				...
				{
					throw new AutomatonExportException("Couldn't parse simple linear expression: " + e.toDefaultString());
				}
				...
				throw new AutomatonExportException("Could not parse operation '" + o.op.toDefaultString() + 
						"' in linear expression: " + e.toDefaultString());
			
			
		}
		else
			throw new AutomatonExportException("Could not parse linear expression: " + e.toDefaultString());
	}
