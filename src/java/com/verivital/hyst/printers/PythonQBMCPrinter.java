 ...
 * outputs a python QBMC model. Based on DReach printer.
 * 
...
public class PythonQBMCPrinter  extends ToolPrinter
{
	...
	{		
		printLine(commentChar + "Variable definitions");
                ...
                printLine("all_state_vars = []"); // ALL state variables over ALL iterations
                ...
                printLine("v_time.append( Real('time_' + str(i)))"); // # time variable for each elapse at BMC step k)
		...
		{
			/*if (v.equals("t")) {
				String vNew = "clock"; // TODO: add replacement for clock if it exists
				swapNames.put(v, vNew);
				v = vNew;
			}
                        */
                        printLine(v + " = []");
	...
	private void printInitialStates() 
        ...
        // init is a mapping: Loc -> expr: need to modify each Expr: put("time", new Constant(0)); // TODO: generalize?
        String text = "";  
	...
	private void printGoalStates() 
	{
		...
		// TODO: pull in bad states from input automaton, currently setting to max of modeBitSize
		//printLine("badStates.append(mode[i] == BitVecVal(pow(2,modeBitSize)-1, modeBitSize))");
                String text = "And(";
                ...
                printLine("badStates.append(" + text  + "mode[i] == BitVecVal("                              
                                + ModeNamesToIds.get(config.forbidden.keySet().iterator().next()) + ", modeBitSize), " 
                                + badvalue + "))");       
		...
		increaseIndentation();
		printLine("badStates");
		//outStream.print(printFlowRangeConditions(ha.initialStates, true, true));
	...
	private void printModes() 
	{
			...
			ModeNamesToIds.put(e.getKey(), count++); 
                ...
                for (Entry <String, AutomatonMode> e : ha.modes.entrySet()) {
                        ...
                        for (String v : ha.variables)
                        {
                                ...
                                {
                                        if (entry.getKey().matches(v) & !entry.getValue().asExpression().toString().equals("0.0")){
                                        //double value = Double.parseDouble(entry.getValue().asExpression().toString());
                                        //if (entry.getKey().matches(v) & (value != 0)){
                                            printLine("next_"+v+" == "+"cur_"+v+" + time * (" + entry.getValue().asExpression() + "),");
                                            match = true;
                                        }  
                                } 
                                if(!match){                                    
		...
		mode_length = ModeNamesToIds.size();
                printLine("Transitions = And("); increaseIndentation();
		...
		for (Entry <String, AutomatonMode> e : ha.modes.entrySet()) 
		{
			...
			line += "Implies(cur_mode == BitVecVal("+ ModeNamesToIds.get(locName) +", modeBitSize), ";
			printLine(line);
                        line = "";
		...
		// discrete identity (modes / non-changing continuous variables are fixed over transitions)
        //for (String v : ha.variableNames) {
        	//printLine("next_"+v+" == cur_"+v+"[i],"); // TODO: wrong, this does for all cont. vars, need to check identities vs. not
        //}
        ...
        private void printQBFMultiplexers()
        {
                ...
                printLine("for i in range(0, k):"); increaseIndentation();
                printLine("if i == 0:"); increaseIndentation();
                printQBFSubMux("v_time[i+1] >= v_time[i]"); decreaseIndentation();
                printLine("else:"); increaseIndentation();
                printQBFSubMux("v_time[i] > v_time[i-1]"); decreaseIndentation();

    
                printLine("muxes.append( mux_i )"); decreaseIndentation();
        ...
        private void printFooter()
        {
                ...
                increaseIndentation();
                printCounterExample(); decreaseIndentation();
        }
	...
	/*
	 * TODO Stan removed this because it was for the old internal format
	 * private String printFlowRangeConditions(Expression ex, boolean isAssignment, boolean isInitAssignment, boolean endLine)
	{
		// HACK: drop spaceex location constraints from expressions (e.g., goal/init)
		if (ex.toString().startsWith("loc(")) {
			return "True";
		}
		...
		if (ex instanceof Operation)
		{
			...
			// stupid mix of infix and prefix
			switch (o.op) {
				case MULTIPLY :
				case DIVIDE :
				case ADD :
				...
				case GREATEREQUAL :
				case NOTEQUAL :
					return printFlowRangeConditions(o.getLeft(), isAssignment, isInitAssignment, endLine) + " " +
					operatorToString(op) + " " + 
					printFlowRangeConditions(o.getRight(), isAssignment, isInitAssignment, endLine);
				case AND :
				case OR :
				default :
					return operatorToString(op) + "(" + 
							" " + printFlowRangeConditions(o.getLeft(), isAssignment, isInitAssignment, endLine) + ", " +
							printFlowRangeConditions(o.getRight(), isAssignment, isInitAssignment, endLine) + ")";
			}
		}
		else if (ex instanceof Variable) {
			// swap illegal variable names
			String varName = ex.toString();
			if (varName == null || varName.equals("null")) {
				throw new AutomatonExportException("Bad variable name: " + varName + " vs. " + ex); 
			}
			
			if (this.swapNames.containsKey(varName)) {
				return this.swapNames.get(varName);
			}
			else {
                                if (((Variable)ex).toString().equals("true"))
                                    return "True";
                                else if (((Variable)ex).toString().equals("false"))
                                    return "False";
                                else if (isInitAssignment)
                                    return ((Variable)ex).toString()+"[0]";
                                else
                                    return "cur_"+((Variable)ex).toString();
			}
		}
			...
			throw new AutomatonExportException("Bad expression printing: " + ex);
		}
	}*/
	...
	{	
        ...
        for (AutomatonTransition transition : ha.transitions)
		{
			...
			//printLine(commentChar + " " + fromName + " -> " + toName + " (" + this.ModeNamesToIds.get(fromName) + " -> " + this.ModeNamesToIds.get(toName) + ")");
			...
			if (guard != null) { 
				// Stan TODO you'll probably want a custom printer so guards print correctly                            
				text += indentation + "And("+guard+", And(";
			} else {
				text += indentation + "And(True, And(";
                        }
			
			// TODO Stan: Tried to change this to try to match the new automaton format
			//Expression.expressionPrinter = curExpressionPrinter;
			...
			/*Expression reset = transition.reset;
			if (reset != null)
			{
				outStream.print(getAssignments(reset));
			}
			else {
				throw new AutomatonExportException("Identity resets was null, but it should not be null: " + reset); 
			}*/
                        
			text += "next_mode == BitVecVal("+ this.ModeNamesToIds.get(toName) +", modeBitSize)))";
		}
                //increaseIndentation();
	 ...
	 */
	/*
	 * TODO: Stan removed this since it's for the old format
	 * private String getAssignments(Expression e)
	{
		String result = "";
		if (e instanceof Operation)
		{
			...
			if (o.getOperator() == Operator.AND)
			{
				result += getAssignments(o.getLeft()) +
							 getAssignments(o.getRight());
			}
			else if (o.getOperator() == Operator.EQUAL)
			{
				// should be x' := EXPR
				if (o.getLeft() instanceof Variable){
					// note: already primed
                                        Variable var = ((Variable)o.getLeft());
                                        var.setPrimed(false);
                                        String stringOfRightSide = ""+o.getRight();
                                        if (!var.toString().equals(stringOfRightSide)) {
                                            // variables in the expression need to be modified to include the cur_ prefix
                                            // I do this by splitting up the expression and modifying each variable one at a time
                                            String[] tokens;
                                            tokens = stringOfRightSide.split("((?<=[^A-Za-z0-9_])|(?=[^A-Za-z0-9_]))");
                                            for (int k = 0; k < tokens.length; k++) {
                                                for (String v : ha.variableNames) {
                                                    if (tokens[k].equals(v)) {
                                                        tokens[k] = "cur_"+v;
                                                        break;
                                                    }
                                                }
                                            ...
                                            for(String str : tokens) sb.append(str);
                                            result += ("next_"+o.getLeft() + " == " + sb.toString()) + ", ";
                                        } else {
                                            result += ("True") + ", ";
                                        }
                                }else
					throw new AutomatonExportException("Assignments should be of the form \"next_<variable> = <expression>\": " + e);
			} else {
                            throw new AutomatonExportException("Assignments should only have ANDs: " + e);
                        }
		}
		else {
			throw new AutomatonExportException("Not an operation: " + e);
		}
		return result;
	}*/
	...
	/*private void printFlowAssignments(Expression expression) 
	{
		if (expression != null) 
		{
			if (expression instanceof Operation) 
			{
				...
				if (op.getOperator() == Operator.EQUAL) 
				{
					...
					{
						String varName = ((Variable) left).get();
						
						if (this.swapNames.containsKey(varName)) {
							varName = this.swapNames.get(varName);
						}
						
						printLine("d/dt[" + varName + "] = " + right + ";");
					}
					else 
						throw new AutomatonExportException("Flow (derivative) left-hand-side is not a variable: " + expression);
				} 
				else if (op.getOperator() == Operator.AND) 
				{
					printFlowAssignments(op.getLeft());
					printFlowAssignments(op.getRight());
				}
				else
					throw new AutomatonExportException("Flows (derivatives) may only containt ANDs: " + expression);
			}
			else
				throw new AutomatonExportException("Flows (derivatives) must be expressions: " + expression);
		}
	}*/
