...
public class StateflowSpPrinter extends ToolPrinter {
    ...
    public final VariableWrapper V_backtrack =
    		new VariableWrapper("a_backtrack");
    ...
    /*
     * NOTE: this is a bug in Stateflow:
     * it REQUIRES it seems newlines to always be Unix-style,
     * regardless of Host operating system,
     * so e.g., using, System.lineSeparator() does not work
     */
    ...
    protected void printDocument(String originalFilename) 
    {
            ...
            Expression.expressionPrinter = new StateflowSpPrinter.StateflowSpExpressionPrinter(0); // TODO: move to constructor?
    ...
    */
    ...
    /**
     * Wrapper for a variable in Stateflow.
     * It contains a name and an iterable of properties to be set.
     */
    public class VariableWrapper {
         ...
         * @param arrSize size of the array (typically an empty string)
         */
        private void setStandardProperties(final String scope,
                final String update, final String datatype,
	...
	private class IntervalBound extends VariableBound {
		...
		public IntervalBound(final String var,
				@Nullable final Expression lower,
				@Nullable final Expression upper) {
			super(var, false);
			this.lower = lower;
			this.upper = upper;
		}
	...
	private class EqualityBound extends VariableBound {
		...
		/**
		 * @param var variable name
		 * @param expr equality expression (including the variable and equality)
		 */
     ...
     */
    public class StateflowSpExpressionPrinter
            extends DefaultExpressionPrinter {
        ...
        /*
         * pretty printing mode: break AND/OR expressions exceeding this threshold
         * deactivated for value 0
         */
         ...
         * @param isAddPrefix true iff variable prefix should be used
         * @param isIntervalEquality true iff equalities should become intervals
         * @param isAddEpsilon true iff epsilons should be added to inequalities
         */
            ...
            switch (op) {
                case EQUAL:
                    if (m_isIntervalEquality) {
                        /*
                         * special handling for equalities via intervals
                         * (lhs - eps <= rhs && lhs + eps >= rhs)
                         * 
                         * NOTE: This relies on the fact that an equality never
                         * occurs below (as a child of) another equality, i.e.,
                         * we have no equality between Booleans as in
                         * "true == false".
                         */ 
                        ...
                        final Operation minus = new Operation(
                                childLeft, Operator.SUBTRACT, m_epsilon);
                        final Operation plus = new Operation(
                                childLeft, Operator.ADD, m_epsilon);
                        final Operation leq = new Operation(minus,
                                Operator.LESSEQUAL, childRight);
                        final Operation geq = new Operation(plus,
                                Operator.GREATEREQUAL, childRight);
                ...
                case GREATEREQUAL:
                    ...
                    if (m_isAddEpsilon) {
                        ...
                        final List<Expression> children = operation.children;
                        final Operation addition = new Operation(
                                children.get(1), epsilonOperator, m_epsilon);
                        operation =
                        		new Operation(children.get(0), op, addition);
                    }
                    break;
                case AND:
                case OR:
                    ...
                    if (m_prettyPrintThreshold > 0) {
                        ...
                        /*
                         * recursively print children first
                         * NOTE: If the children are split, then the parents are
                         * always split, too. This is because the String object
                         * returned has no notion of line breaks. This is the
                         * intended behavior, however.
                         */
                        ...
                        if (Math.min(lhs.length(), rhs.length()) >=
                        		m_prettyPrintThreshold) {
                            // - decided to split the string -
                            
                            // add parentheses depending on operator priority
                            final int priority = Operator.getPriority(op);
                            final boolean addParenthesesLeft =
                                (leftChild instanceof Operation)
	                                ? (priority > Operator.getPriority(
                                        ((Operation)leftChild).getOperator()))
                                    : false;
                            final boolean addParenthesesRight =
	                            (rightChild instanceof Operation)
                                    ? (priority > Operator.getPriority(
                                        ((Operation)rightChild).getOperator()))
                                    : false;
     ...
     * 
     * @param variableNames iterable of variable names
     * @param m_isAddEpsilon true iff epsilons should be added to inequalities
     * @param prettyPrintThreshold threshold for breaking long expressions
     */
    public StateflowSpPrinter(final Iterable<String> variableNames,
        ...
        /*
         * forces unix-style line endings (Matlab requires this)
         * (DID NOT WORK TO SET to override System.lineSeparator(),
         * maybe have to use old style of System.get etc.)
         */
    ...
    /**
     * Converts the dynamics of a mode to semantics-preserving strings.
     * For each variable that is changed, the returned object contains one
     * string determining the dynamics.
     * Additionally, the output variables are assigned.
     * 
    ...
    private Iterable<String> getDynamicsIterable(final AutomatonMode mode) {
        return getUpdate(mode.flowDynamics, PREFIX_VARIABLE,
            PREFIX_OUTPUT, "_dot = ");
    }
    ...
    private Iterable<String> getInitIterable(final AutomatonMode mode,
        ...
        final Iterable<VariableBound> bounds =
        		getBounds(getConjuncts(initExpr), constants);
...
//        	System.out.println("The variables are bounded.");
        ...
        for (final VariableBound bound : bounds) {
        	if (bound instanceof EqualityBound) {
        		// equality constraint
        		inits.add(m_printer.print(((EqualityBound)bound).expr,
        				true, false, false) + ";");
        		...
        		builder.append("), ");
        		builder.append(
        				m_printer.print(intBound.lower, true, false, false));
        		builder.append(", ");
				builder.append(
						m_printer.print(intBound.upper, true, false, false));
				builder.append(");");
     ...
     * @return iterable of Stateflow strings
     */
    private Iterable<String> getAssignmentIterable(
    		final AutomatonTransition transition) {
        return getUpdate(transition.reset, PREFIX_ASSIGNMENT,
            PREFIX_VARIABLE, " = ");
    }
    ...
    /**
     * Converts an update expression to semantics-preserving strings.
     * An update expression is an expression like a flow or an assignment.
     * 
     * @param map map from variable name to update expression
     * @param prefix1 prefix for first variable
     * @param prefix2 prefix for second variable
     * @param connector1 connector for first 
     */
    private Iterable<String> getUpdate(final Map<String,
    		ExpressionInterval> map, final String prefix1, final String prefix2,
    		final String connector1) {
        // empty case
        if (map.isEmpty()) {
        ...
        for (final Entry<String, ExpressionInterval> entry : map.entrySet()) {
            ...
            final String value = m_printer.print(
                    entry.getValue().asExpression(), true, false, false);
            if (!isSP){
    ...
    /**
     * Converts a conditional expression to a semantics-preserving string.
     * A conditional expression is an expression like an invariant or a guard.
     * 
     ...
     * @param numTransOut number of outgoing transitions
     */
    public String getStateInLabel(final AutomatonMode mode, final int modeId,
            final int numTransOut) {
        final StringBuilder builder = new StringBuilder();
     ...
     * 
     * @param numTransMax maximum number of transitions
     * @param numInits number of initial locations
     */
    public String getTransitionInitLabel(final int numTransMax,
            final int numInits) {
        final StringBuilder builder = new StringBuilder();
        ...
        if (numInits > 1) {
            ...
            if (numInits != 1) {
                throw new IllegalArgumentException(
                        "There was no initial location specified.");
            }
            builder.append(" = 1;");
        }
        builder.append("}");
    ...
    /**
     * Returns the label for the transition
     *  'choose state -> transition choose junction'.
     */
    ...
    /**
     * Returns the label for the transition
     *  'time choose junction -> dwell state'.
     */
    ...
    /**
     * Returns the label for the transition
     *  'leave junction -> transition junction'.
     * 
    ...
    /**
     * Returns the label for the transition
     *  'transition junction -> entry state'.
     * A transition may contain a guard and an assignment, and the target
     * location may have an invariant.
     * 
     * @param transition automaton transition
     */
    public String getTransitionTrans2inLabel(
    		final AutomatonTransition transition) {
        // guard string
     ...
     * 
     * @param expr expression in conjunctive normal form
     */
    private Iterable<Operation> getConjuncts(final Expression expr) {
        if (! (expr instanceof Operation)) {
            throw new IllegalArgumentException(
                    "The formula should be a proper operation.");
        }
        ...
        while (! stack.isEmpty()) {
            ...
            if (formula.getOperator() == Operator.AND) {
                ...
                assert ((formula.getLeft() instanceof Operation) &&
                    (formula.getRight() instanceof Operation)) :
                        "The subformulae should be proper operations.";
    ...
    /**
     * Reads variable bounds from a series of conjuncts and puts them into
     * an internal format.
     * 
     ...
     * <code>var2 = var1 & var2 = 5</code>
     * must be given in the form
     * <code>var2 = 5 & var1 = var2</code>
     * so we can write
     * <code>var2 = 5; var1 := var2</code>
     ...
     * @param constants constants map
     */
    private Iterable<VariableBound> getBounds(
    		final Iterable<Operation> expressions,
    		final Map<String, Double> constants) {
    	final LinkedHashMap<String, VariableBound> bounds =
    			new LinkedHashMap<String, VariableBound>();
    		...
    		if (isVarLeft) {
	    		...
	    		if (isVarRight) {
	    			...
	    			if (isConstantVariable) {
	    				/*
	    				 * One of the expressions was a constant variable.
	    				 * Here we can simply ignore the expression.
	    				 */
...
//	    						conjunct);
		    			...
		    			throw new IllegalArgumentException(
	    					"One side of an operation must be a (non-constant) "
	    					+ "variable.");
	    			}
	    		}
    		}
    		assert (varName != null) :
    			"The null case should lead to an exception.";
    		...
    		switch (conjunct.getOperator()) {
    			case EQUAL:
    				...
    				if (isVarLeft) {
    					...
    					eqBound = new EqualityBound(varName,
    							new Operation(rhs, Operator.EQUAL, lhs));
    				}
    ...
    private boolean checkBoundedness (final Iterable<VariableBound> bounds) {
    	for (final VariableBound bound : bounds) {
    		// equality constraint
    		if (bound instanceof EqualityBound) {
    			assert (((EqualityBound)bound).expr != null) :
    				"Equality should be set.";
    			continue;
    		} else {
    			final IntervalBound intBound = (IntervalBound)bound;
    ...
    /**
     * Recursively applies substitution to an expression.
     * This method converts the input expression into an expression where each
     * occurrence of a variable is replaced by the respective expression defined
     * in the substitution map.
     * 
     * @param expr raw expression
     * @param var2replace map (variable name -> expression)
    ...
    private Expression substitution(final Expression expr,
        ...
        if (expr instanceof Variable) {
            final ExpressionInterval replace =
                    var2replace.get(((Variable)expr).name);
            if (replace != null) {
     ...
     * 
     * @param isStore true iff string for storing is wanted
     * @param vars variables
     */
    private String getStoreString(final boolean isStore,
    ...
    private String getWpString(final AutomatonTransition transition) {
        if (transition.to.invariant == null) {
            throw new IllegalArgumentException(
                    "The invariant should be non-empty.");
        }
        if (transition.reset.isEmpty()) {
            throw new IllegalArgumentException(
                    "The assignment should be non-empty.");
        }
        
        final Expression wp =
                substitution(transition.to.invariant, transition.reset);
        return getCondition(wp);
    }
    
    /**
     * Returns the auxiliary variables introduced by the translation.
     * This function can only be called after the number of maximum transitions
     * was found.
     * 
     * NOTE: A call to this function initializes some data structures.
     * If the function is called more than once, it should be changed to store
     * the variables to avoid recomputation efforts.
     * 
     * @param numTransMax maximum number of transitions
     * @param numRandMax maximum number of random variables used
     * @return iterator over all auxiliary variables
     */
    public Iterator<VariableWrapper> getVariables(final int numTransMax,
    		final int numRandMax) {
        ...
        final List<VariableWrapper> variables = 
        		new LinkedList<VariableWrapper>();
        ...
        V_backtrack.setStandardProperties(
        		SCOPE_O, UPDATE_D, DATATYPE_I, ARR_EMPTY);
        ...
        V_transIdx.setStandardProperties(
        		SCOPE_L, UPDATE_D, DATATYPE_I, ARR_EMPTY);
        ...
        V_resets.setStandardProperties(
        		SCOPE_L, UPDATE_D, DATATYPE_I, ARR_EMPTY);
        variables.add(V_resets);
        
        V_maxResets.setStandardProperties(
        		SCOPE_P, UPDATE_D, DATATYPE_I, ARR_EMPTY);
        ...
        if (numTransMax > 0) {
        	...
        	V_transArray.setStandardProperties(
            		SCOPE_L, UPDATE_D, DATATYPE_A, arrSize);
            variables.add(V_transArray);
            
            V_randArray.setStandardProperties(
            		SCOPE_I, UPDATE_D, DATATYPE_A, arrSize);
