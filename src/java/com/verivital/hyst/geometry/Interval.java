...
public class Interval
{
	...
	/**
	 * Get the constant value of this interval. If this interval is not a constant, then an AutomatonExportException is 
	 * raised. You can check if it's a constant by calling isPoint() or isConstant()
	...
	public double asConstant()
	{
		...
		if (isPoint())
			...
			throw new AutomatonExportException("Interval.asConstant() called on an Interval that had a range: " + this);
	...
	public boolean contains(Interval i) { return min <= i.min && i.max <= max; }
	...
	public boolean isPoint()
	{
		...
		if (!Double.isInfinite(max) && !Double.isNaN(max) && (max == min || (max >= min && max - min < TOL)))
			rv = true;
	 ...
	 * @param ranges the ranges for any variables in e
	...
	public static Interval intervalEvaluate(Expression e, Map<String, Interval> ranges)
	{
		...
		{
			throw new AutomatonExportException("Error performing interval evaluation on expression: " + e, ex);
		}
	...
	private static Interval intervalEvaluateRec(Expression e, Map<String, Interval> ranges)
	{
		...
		if (e instanceof Variable)
		{
			...
			if (i == null)
				throw new AutomatonExportException("Range of variable " + varName + " is needed, but wasn't provided.");
		...
		else if (e instanceof Operation)
		{
			...
			case DIVIDE:
			{
				if (right.min <= 0 && right.max >= 0)
					throw new AutomatonExportException("Interval division contains zero in denominator.");
			...
			case LN:
			{
				...
				if (i.min <= 0)
					throw new AutomatonExportException("Interval evaluate of ln with min <= 0: " + i.min);
			...
			case SQRT:
			{
				...
				if (i.min < 0)
					throw new AutomatonExportException("Interval evaluate of sqrt with min < 0: " + i.min);
			...
			case COS:
			case TAN:
				// these could be done using Algorithm 2 on page 29 of Xin Chen's dissertation
				// tan would need division tan(x) = sin(x) / cos(x)
				throw new AutomatonExportException(
						"Operator is not yet supported (submit a feature request if you need it): " + o.op.name());
	...
	/**
	 * Compute the power function for intervals. Based on Algorithm 1 from Xin Chen's dissertation (Page 28)
	 * @param left the left interval
	 * @param right the right interval (must be a nonnegative constant);
	...
	private static Interval intervalPow(Interval left, Interval right)
	{
		...
		if ((val != Math.floor(val)) || Double.isInfinite(val) || val < 0) 
			throw new AutomatonExportException("Only nonnegative integer exponents are supproted: " + val);
