 ...
 *
 * automaton is guaranteed nonnull
 * plotVariableNames is guaranteed nonnull, and of size 2, although the elements may be null (if the model
 * contains no variables, for example) 
 * 
...
public class AutomatonSettings
{
	...
	{
		return "[AutomatonSettings plotVariableNames = " + plotVariableNames[0] + ", " + plotVariableNames[1] + "]";
	}
	...
	public void validate()
	{
		...
		{
			if (plotVariableNames[i] != null && !config.root.getAllVariables().contains(plotVariableNames[i]))
				throw new AutomatonValidationException("plot variable not in automaton: " + plotVariableNames[i]);
		}
