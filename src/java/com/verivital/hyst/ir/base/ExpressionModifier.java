...
public abstract class ExpressionModifier
{
	...
	private static LinkedHashMap <String, ExpressionInterval> modifyMap(
			LinkedHashMap <String, ExpressionInterval> m,
			ExpressionModifier em)
	{
		...
		for (Entry<String, ExpressionInterval> e : m.entrySet())
		{
			...
			rv.put(e.getKey(), new ExpressionInterval(em.modifyExpression(ei.getExpression()), ei.getInterval()));
		}
	 ...
	 * @param eim the modification to perform 
	 */
	...
	/**
	 * Enumerate over all the expressions in this base component and modify them with the passed-in object
	 * @param ha the automaton to enumerate over
	 * @param em the function that gets called to modify each expression
	 */
