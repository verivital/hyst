 ...
 * 
 * automaton, from and to, and guard are nonnull
 * reset is nonnull as well, but may be empty
 * Label may be null (if no label), but not blank ("")
 * 
...
public class AutomatonTransition
{
	...
	public LinkedHashMap <String, ExpressionInterval> reset; // if x' := x + 1, then x maps to x + 1 
	...
	{
		// parent may be different, so search for from.name and to.name in parent
