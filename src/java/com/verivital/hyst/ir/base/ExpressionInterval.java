...
public class ExpressionInterval {
	...
	public boolean equals(Object o) {
		...
		if (o instanceof ExpressionInterval) {
			...
			rv = toString(DefaultExpressionPrinter.instance).equals(
					other.toString(DefaultExpressionPrinter.instance)); // not
																		// ideal
																		// but
																		// it
																		// works
	...
	public String toString(ExpressionPrinter printer) {
		...
		if (i != null)
			rv += " " + printer.printOperator(Operator.ADD) + " ["
					+ printer.print(new Constant(i.min)) + ", "
					+ printer.print(new Constant(i.max)) + "]";
	 ...
	 * @param variables
	 *            the ordering of variables within hp
	...
	public Interval evaluate(HyperPoint hp, ArrayList<String> variables) {
		...
		if (Double.isInfinite(val) || Double.isNaN(val))
			throw new AutomatonExportException(
					"ExpressionInterval.evaluate() yielded NaN or infinity");
