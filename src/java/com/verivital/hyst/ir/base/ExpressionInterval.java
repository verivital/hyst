...
public class ExpressionInterval
{
	...
	/**
	 * Get the Expression which equals this ExpressionInterval exactly. This is only valid if min==max==0, otherwise
	 * an AutomatonExportException is raised.
	 * 
	 * @return the Expression
	 * @throws AutomatonExportException if the interval is not [0,0]
	 */
	public Expression asExpression()
	{
		if (i != null)
		{
			...
			if (i.isPoint() && e instanceof Constant)
			{
				...
				throw new AutomatonExportException("ExpressionInterval.asExpression called, but interval is nonnull and" +
						" not convertable: " + this + " (is there explicit support for ExpressionIntervals?)");
		}
		return e;
	}
	
	/**
	 * Default printing. If you want something different you'll need to manually write it externally as this type
	 * is not part of ExpressionPrinter
	 */
	...
	/**
	 * Add a range to the range defined in this ExpresionInterval. This has the effect of min = left.min + right.min,
	...
	public boolean equals(Object o)
	{
		...
		if (o instanceof ExpressionInterval)
		{
			...
			rv = toString(DefaultExpressionPrinter.instance).equals(
					other.toString(DefaultExpressionPrinter.instance)); // not ideal but it works
		}
	...
	{
		if (i != null) {
			return e.equals(new Constant(0)) && i.equals(Interval.NONDETERMINISTIC_ASSIGNMENT_INTERVAL);
		}
		else {
			return false;
		}
