 ...
 * do renaming of variables, labels, constants, ect.
 * 
 * The children of this class are the instantiated sub-components, not the templates. Thus, if a single base component is
 * instantiated several times, there will be several children here.
 * 
 * Validation guarantees:
 * children is not null and size > 0, instance names are valid identifiers, Components are nonnull 
 * 
 */
public class NetworkComponent extends Component
{
	...
	public void validate()
	{
		...
		if (children == null || children.size() == 0)
			throw new AutomatonValidationException("children was null or empty in network component");
		...
		for (Entry<String, ComponentInstance> e : children.entrySet())
		{
			...
			if (!e.getKey().equals(ci.child.instanceName))
				throw new AutomatonValidationException("Instance name in child doesn't match name in NetworkComponent");
		}
	}
