...
public class ComponentMapping
{
	...
	public String parentParam; // the parameter in the parent (may be a ~constant during template instantiation)
	...
	{
		if (childParam == null)
			throw new AutomatonValidationException("childParam is null in mapping of " + mappingType + " in component " 
					+ instanceName);
		
		if (parentParam == null)
			throw new AutomatonValidationException("parentParam is null in mapping of " + mappingType + " in component " 
					+ instanceName);
		
		if (Component.VALIDATE_CONSTS || !parentParam.startsWith(TemplateImporter.TEMPLATE_VALUE_MAP_PREFIX))
		{
			if (!parentList.contains(parentParam))
				throw new AutomatonValidationException("parent component doesn't contained mapped " + mappingType + " for child '" 
					+ parentParam + "' in parent component named " + instanceName);
		}
		
		if (!childList.contains(childParam))
			throw new AutomatonValidationException("child component doesn't contained mapped " + mappingType + " '" + childParam 
