 ...
 * variable / constant / label names of the parent to those of the child.
 * 
 * Validation guarantees:
 * parent and child are nonnnull
 * childTemplateName is nonnull
 * 
...
public class ComponentInstance
{
	...
	public void validate()
	{
		...
		if (child.parent != parent)
			throw new AutomatonValidationException("child.parent not equal to the parent in the component instance");
	...
	public ComponentInstance copy()
	{
		...
		for (ComponentMapping c : constMapping)
		{
			if (c.parentParam.startsWith(TemplateImporter.TEMPLATE_VALUE_MAP_PREFIX))
			{
				...
				String varStr = c.parentParam.substring(TemplateImporter.TEMPLATE_VALUE_MAP_PREFIX.length());
				try
				{
				...
				{
					throw new AutomatonExportException("Couldn't parse constant in ValueMap of parameter '" + c.childParam + 
							"': " + varStr, e);
				}
			}
			else
