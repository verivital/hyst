...
public class UIWaypoints {
	 ...
	 * @param index Number of the waypoint (starting at 0)
	 * @param isBefore Get a point BEFORE (true) or AFTER (false) the arrow's
	 * 		middle point
	...
	public UIPosition getWaypoint(int index, boolean isBefore) {
		ArrayList<UIPosition> positions =
				isBefore ? mBeforeMiddle : mAfterMiddle;
	 ...
	 * @param isBefore Get the number of points BEFORE (true) or AFTER (false)
	 * 		the arrow's middle point
	 * @return The number of waypoints in the requested list.
	 */
	public int getCount(boolean isBefore) {
		if (isBefore) return mBeforeMiddle.size();
		return mAfterMiddle.size();
	}

	/**
	 * Add a new waypoint
	 * @param waypoint The new waypoint's position
	 * @param insertBefore Insert into the list of points BEFORE (true) or AFTER
	 * 		(false) the arrow's middle point
	 */
	public void addWaypoint(UIPosition waypoint, boolean insertBefore) {
		ArrayList<UIPosition> positions =
				insertBefore ? mBeforeMiddle : mAfterMiddle;
		positions.add(waypoint);
	}

	/**
	 * Add a new waypoint
	 * @param X The new waypoint's X position
	 * @param Y The new waypoint's Y position
	 * @param insertBefore Insert into the list of points BEFORE (true) or AFTER
	 * 		(false) the arrow's middle point
	 */
