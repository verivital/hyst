 ...
 * - Locations with Transitions (Base Component)
 * or
...
public abstract class SpaceExComponent {
	 ...
	 * @param index Number of the Param, starting at 0
	 * @return The requested Param OR null if the index is out of bounds.
	 */
	public Param getParam(int index) {
		if ((index < 0) || (index >= mParams.size())) return null;
		return mParams.get(index);
	}
	
	/**
	 * Get a Param by it's name.
	 * @param id The Param's name.
	 * @return The requested Param
	 * 		OR null if there is no Param with the wanted name.
	 */
